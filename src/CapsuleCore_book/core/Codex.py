...
class Codex:
    ...
    def create_entry(self, title: str, content: str, tags: List[str] = None, category: str = None, **kwargs) -> Entry:
    ...
    def create_relation(self, from_id: str, to_id: str, connection_type: str = "relation", **kwargs) -> Relation:
        ...
        if not origin or not target:
            missing = []
            if not origin: missing.append(f"Origen({from_id})")  # noqa: E701
            if not target: missing.append(f"Destino({to_id})")  # noqa: E701
            raise ValueError(f"No se puede crear la relación. Faltan: {', '.join(missing)}")
        ...
        if self.repository.check_relation(new_relation):    
            raise ValueError(f"Ya existe una relación entre {from_id} y {to_id} con tipo '{clean_type}'")
    ...
    def delete_entry(self, entry_id: str) -> bool:
        ...
        if not entry_id or not isinstance(entry_id, str):
            raise ValueError("Se requiere un ID de entrada válido (string) para eliminar.")
    ...
    def disconnect_entries(self, from_id: str, to_id: str, connection_type: Optional[str] = None) -> bool:
    ...
    def groupBy_tags(self, tag: str) -> List[Entry]:
       ...
       #2
        normalized_tag = self._sanitize_tag(tag)
    ...
    def _rules_category(self, category: Optional[str]) -> str:
            ...
            if self.policy.category_max_length:
                processed = self._validate_string_length(processed, self.policy.category_max_length)

            return processed
    
    def _process_field_change(self, field_name: str, new_value: Any, existing_entry: Entry) -> Any:
            ...
            if field_name == 'title':
                ...
                clean_title = self._rules_title(new_value, current_entry_id=existing_entry.id)
    ...
    def _rules_title(self, title: Optional[str], current_entry_id: Optional[str] = None) -> str:
        """
        rules:
        ...
        if self.policy.title_max_length:
            processed = self._validate_string_length(processed, self.policy.title_max_length)
    ...
    def _validate_title_uniqueness(self, title: str, current_entry_id: Optional[str] = None) -> None:
